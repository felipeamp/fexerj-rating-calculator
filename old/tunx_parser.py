...
_BIO_MARKER = bytes.fromhex("a5ff8944")
_PAIRING_MARKER = bytes.fromhex("b3ff8944")
_BYE_SNR = 0xFFFE
_PAIRING_STRIDE = 21
_RESULT_WIN = 1
_RESULT_DRAW = 2
_RESULT_LOSS = 3
_KNOWN_RESULT_CODES = {0, 1, 2, 3, 4, 5, 6, 7, 9}
_UNKNOWN_RESULT_THRESHOLD = 0.05  # flag if >5% of records have unknown result codes


def _read_field(data, offset):
...
def _is_printable_utf16(data, offset, char_count):
...
def _skip_to_binary_block(data, pos, bio_end, min_zeros=8):
...
def _find_next_record(data, pos, bio_end):
    ...
    while pos < bio_end - 10:
        ...
        if 1 <= n1 <= 40 and _is_printable_utf16(data, pos + 2, n1):
            end1 = pos + 2 + n1 * 2
            if end1 + 2 <= bio_end:
                n2 = struct.unpack_from('<H', data, end1)[0]
                if 1 <= n2 <= 50 and _is_printable_utf16(data, end1 + 2, n2):
                    return pos
        pos += 1
    return bio_end


def _validate(filepath, data, bio, games):
    ...
    # Pairing section boundaries (same logic as _parse_pairing_section)
    pairing_start = data.find(_PAIRING_MARKER) + 4
    ...
    if section_size % _PAIRING_STRIDE != 0:
        issues.append(
            f"pairing section size {section_size} is not a multiple of stride {_PAIRING_STRIDE}"
        )

    total_records = section_size // _PAIRING_STRIDE
    unknown_count = 0
    pos = pairing_start
    while pos + _PAIRING_STRIDE <= pairing_end:
        _, _, result = struct.unpack_from('<HHH', data, pos)
        pos += _PAIRING_STRIDE
        if result not in _KNOWN_RESULT_CODES:
            unknown_count += 1
    if total_records > 0 and unknown_count / total_records > _UNKNOWN_RESULT_THRESHOLD:
        issues.append(
...
def parse_tunx(filepath):
    ...
    games = _parse_pairing_section(data)
    _validate(filepath, data, bio, games)
    return bio, games


def _parse_bio_section(data):
    bio_start = data.find(_BIO_MARKER) + 4
    bio_end = data.find(_PAIRING_MARKER)
    ...
    while offset < bio_end - 4:
        try:
            ...
            _club, offset = _read_field(data, offset)
            _fed, offset = _read_field(data, offset)
            offset = _skip_null(data, offset)    # fed may or may not carry a null
        ...
        offset = _skip_to_binary_block(data, offset, bio_end)
        next_start = _find_next_record(data, offset, bio_end)
...
def _parse_pairing_section(data):
    pairing_start = data.find(_PAIRING_MARKER) + 4
    pairing_end = len(data)
    for marker_hex in ("b5ff8944", "d3ff8944", "e3ff8944"):
        pos = data.find(bytes.fromhex(marker_hex))
